...
import worker
from plot_gene_bar_chart import plot_maf
import datetime
...
def processFile(file):
    ...
    # variants_percentage_dict = computePercentageOfVariant(maf_file_name = file, variantColumnName = "Variant_Type")
    return plot_maf(file, "BRCA")
...
def upload():
    ...
    return jsonify({
        "urlToCheck": "http://localhost:5012/v1/pipegene/{}/status".format(step_id),
        "message": "IN_PROGRESS",
    })
...
if __name__ == '__main__':
    worker.start()
    address = "127.0.0.1"
    app.run(host=address, port=5012)
