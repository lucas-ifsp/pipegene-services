...
import worker
from plot_composite_barchart import plot_top10
import datetime
...
def processFile(file):
    ...
    return plot_top10(file, title="BRCA")
...
def upload():
    ...
    return jsonify({
        "urlToCheck": "http://localhost:5017/v1/pipegene/{}/status".format(step_id),
        "message": "IN_PROGRESS",
    })
...
if __name__ == '__main__':
    worker.start()
    address = "127.0.0.1"
    app.run(host=address, port=5017)
